...
def render(
    ...
    # loop throught matrix horizontally-vertically
    for i in range(v_res):
        for j in range(h_res):
            q = q0 + square_side * (j * u - i * v)
            # get pixel color with cast
            color = cast(
                eye, normal((q - eye)), bkgd_color / 255, objects, ca, lights, max_depth
            )
...
def cast(origin, d, color, objects, ca, lights, max_depth):
    # call trace to get list of intersections
    # get list of objects intersected
    ...
    if len(s) != 0:
        ...
        # call shade to get "initial" pixel color
        # then, recursion calls will add to it until recursing limit is reached
        color = shade(closest, p, w0, n, ca, lights, objects)
        if max_depth > 0:
            kr = closest["kr"]
            kt = closest["kt"]
            # if kt > 0, there's recursion to be done
            # kt = constante de refração
            if kt > 0:
                try:
                    ...
                    # recursion call
                    color = color + kt * cast(
                        p_, r, color, objects, ca, lights, max_depth - 1
                    )
                except:
                    kr = 1
                    pass
            # if kr > 0, there's recursion to be done
            # kr = constante de reflexão
            if kr > 0:
                r = reflect(w0, n)
                p_ = p + 1e-5 * r
                color = color + kr * cast(
...
def main():
    ...
    rendered_img = render(*data.values())  # return matrix with rgb elements
